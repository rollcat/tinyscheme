...
INTERFACE long ivalue(pointer p)      {
    return (num_is_integer(p) ? (p)->_object._number.value.ivalue : (long)(
...
static port *port_rep_from_filename(scheme *sc, const char *fn,
                                    int prop);
...
static void atom2str(scheme *sc, pointer l, int f, char **pp,
                     int *plen);
...
static void push_recent_alloc(scheme *sc, pointer recent,
                              pointer extra) {
...
static INLINE pointer oblist_find_by_name(scheme *sc,
        const char *name) {
    int location;
    pointer x;
    char *s;
...
INTERFACE pointer mk_counted_string(scheme *sc, const char *str,
                                    int len) {
    pointer x = get_cell(sc, sc->NIL, sc->NIL);
...
static int file_push(scheme *sc, const char *fname) {
    ...
    if (fin != 0) {
        ...
        if (fname)
            sc->load_stack[sc->file_i].rep.stdio.filename = store_string(sc,
...
static pointer port_from_filename(scheme *sc, const char *fn,
                                  int prop) {
    port *pt;
...
static pointer reverse_in_place(scheme *sc, pointer term,
                                pointer list) {
...
static pointer opexe_0(scheme *sc, enum scheme_opcodes op) {
    ...
    case OP_DEF0:  /* define */
        ...
        if (is_pair(car(sc->code))) {
            x = caar(sc->code);
            sc->code = cons(sc, sc->LAMBDA, cons(sc, cdar(sc->code),
                                                 cdr(sc->code)));
        } else {
            x = car(sc->code);
...
scheme *scheme_init_new_custom_alloc(func_alloc malloc,
                                     func_dealloc free) {
    scheme *sc = (scheme *)malloc(sizeof(scheme));
...
void scheme_load_named_file(scheme *sc, FILE *fin,
                            const char *filename) {
    dump_stack_reset(sc);
    sc->envir = sc->global_env;
    sc->file_i = 0;
...
void scheme_load_string(scheme *sc, const char *cmd) {
    ...
    sc->load_stack[0].rep.string.start = (char *)
                                         cmd; /* This func respects const */
